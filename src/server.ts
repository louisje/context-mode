...
// ── ctx-upgrade: disabled for local/offline install ─────────────────────────
// Upstream registers a tool here that returns a shell command instructing the
// LLM to run an upgrade — either `<cli> upgrade` or, when no local CLI file is
// found, an inline fallback script that `git clone`s the upstream GitHub repo,
// rebuilds it from source, and copies the result over this plugin's install
// directory. That fallback would silently overwrite every risk-behavior
// removal made in this build (version pings, ctx_insight, background npm
// install) with stock upstream code. Removed entirely so no tool in this
// build can trigger a self-overwrite from a remote source.
